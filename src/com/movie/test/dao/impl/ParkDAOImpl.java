...
public class ParkDAOImpl implements ParkDAO {
	...
	public List<Map<String, String>> selectParkList() {
		...
		try {
			String sql = "select tp_num, tp_name, tp_address, tp_phone, ti_num, "
					+ "(select ti_name from theater_info ti where ti.ti_num=tp.ti_num) as ti_name from theater_park tp";
	...
	public Map<String, String> selectPark(int tpNum) {
		...
		ResultSet rs = null;
		try {
			String sql = "select tp_num, tp_name, tp_address, tp_phone, ti_num,(select ti_name from theater_info ti where ti.ti_num=tp.ti_num) as ti_name"
					+ " from theater_park tp where tp_num=?";
			ps = con.prepareStatement(sql);
			ps.setInt(1, tpNum);
			rs = ps.executeQuery();
			while(rs.next()) {
	...
	public static void main(String[] args) {
		...
		List<Map<String,String>> parkList = pDAO.selectParkList();
		System.out.println(parkList);
		Map<String,String> park = pDAO.selectPark(1);
